...
## Pinned Facts
*Soft facts the GM always keeps — canon the table never wants forgotten. Read at every /gm load alongside GM Style Notes and kept hot all session. Unlike Live State Flags (which track shifting state and are rewritten each save), pinned facts are stable and are only added or removed on request. Pin a promise made, a dead sibling's name, a running in-joke, a table house rule, a detail the player has said matters. Add/list/remove with `/gm pin`.*
*(none pinned yet)*
