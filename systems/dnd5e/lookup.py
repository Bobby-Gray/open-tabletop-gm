...
def _fmt_monster(r: dict) -> str:
    ...
    # Defenses before languages: these change how a fight resolves, and a GM
    # scanning this block mid-combat is looking for them, not for what the
    # creature speaks. Omitted entirely when empty rather than printed as
    # "Resistances: —", so an absent line means "none" and never "unknown".
...
def main() -> None:
    ...
    if not results:
        ...
        hints = suggest(query, category=category if category in CATEGORY_MAP else None)
        if hints:
            pretty = ", ".join(
