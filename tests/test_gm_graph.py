"""
test_gm_graph.py — end-to-end smoke tests for the gm_graph CLI.

Builds an ephemeral campaign in /tmp, runs the actual `gm_graph.py`
...
Run from repo root:
    python3 -m unittest tests.test_gm_graph -v
"""
import json
import os
import pathlib
...
SCRIPT = REPO / "scripts" / "gm_graph.py"


def _run(args, env_overrides=None, stdin_text=""):
    """Run gm_graph.py with args, return (returncode, stdout, stderr)."""
    env = os.environ.copy()
    if env_overrides:
        env.update(env_overrides)
    r = subprocess.run(
        [sys.executable, str(SCRIPT), *args],
        capture_output=True, text=True, env=env, input=stdin_text, encoding="utf-8",
    )
...
class GraphSubcommandTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {"GM_CAMPAIGN_ROOT": str(self.root)}
    ...
    def test_extract_deterministic_against_synthetic_log(self):
        """Run the actual subcommand: extract produces JSON (OTGM is deterministic-only)."""
    ...
    def test_extract_deterministic_category_object_creates_category_node(self):
        """Phase 2.5: state-verbs with category_object_ok extract category-target edges (OTGM)."""
        (self.camp_dir / "session-log.md").write_text(textwrap.dedent("""\
            # Session Log
    ...
    def _active_stance(self, graph, to_id):
        """Return the active (unclosed, un-superseded) party→to_id edges."""
