...
Server-side wrapper called by /tts in gm-display-app.py. Reads the API key
from GM_TTS_KEY env var → GEMINI_API_KEY env var → ~/.config/open-tabletop-gm/tts.key
...
KEY_FILE = Path.home() / ".config" / "open-tabletop-gm" / "tts.key"
...
# truncate. Bigger blocks should be chunked by the caller; the GM display
# narration path naturally chunks at block flush.
MAX_TEXT_CHARS = 2000
...
def _get_api_key() -> Optional[str]:
    ...
    for env in ("GM_TTS_KEY", "GEMINI_API_KEY"):
...
def _cli() -> int:
    ...
    p = argparse.ArgumentParser(
        description="Verify Gemini Flash TTS setup for the GM display companion.",
    )
    p.add_argument(
